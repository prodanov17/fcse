...
if __name__ == '__main__':
    ...
    dataset_y = [map_quality(row[-1]) for row in dataset]

    test_x, train_x, test_y, train_y = train_test_split(dataset_x, dataset_y, test_size=test_size, shuffle=False)
    # train_x, test_x, train_y, test_y = train_test_split(dataset_x, dataset_y, test_size=test_size, shuffle=False)
    ...
    least_important_index = features.index(min(features))
    # train_x = [[row for index,row in enumerate(row) if index != least_important_index] for row in train_x]
    # test_x = [[row for index,row in enumerate(row) if index != least_important_index] for row in test_x]
    train_x = [[row[i] for i in range(len(train_x[0])) if i != least_important_index] for row in train_x]
    test_x = [[row[i] for i in range(len(test_x[0])) if i != least_important_index] for row in test_x]

    mlp_model = MLPClassifier(hidden_layer_sizes=15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)

    scaler = StandardScaler()
    train_x_std = scaler.fit_transform(train_x)
    test_x_std = scaler.transform(test_x)

    mlp_model.fit(train_x_std, train_y)
    predictions_std = mlp_model.predict(test_x_std)
    print(f"Tocnost so StandardScaler: {accuracy_score(test_y, predictions_std)}")

    # Train MLPClassifier with MinMaxScaler
    scaler2 = MinMaxScaler()
    train_x_mm = scaler2.fit_transform(train_x)
    test_x_mm = scaler2.transform(test_x)

    mlp_model.fit(train_x_mm, train_y)
    predictions_mm = mlp_model.predict(test_x_mm)
    print(f"Tocnost so MinMaxScaler: {accuracy_score(test_y, predictions_mm)}")
