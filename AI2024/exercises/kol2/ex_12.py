...
def hasOverfitting(train_accuracy, test_accuracy):
    return train_accuracy > test_accuracy * 1.15
...
if __name__ == '__main__':
    ...
    train_x, test_x, train_y, test_y = train_test_split(dataset_x, dataset_y, test_size=0.2, shuffle=False)
    ...
    model = MLPClassifier(hidden_layer_sizes=hidden_layers, learning_rate_init=learning_rate, activation="relu", max_iter=20, random_state=0)
    ...
    if hasOverfitting(accuracy_train, accuracy):
        train_x = [[row[i] for i in range(len(train_x[0])) if i != col_index] for row in train_x]
        test_x = [[row[i] for i in range(len(test_x[0])) if i != col_index] for row in test_x]

        sample = [row for ind,row in enumerate(sample) if ind != col_index]
