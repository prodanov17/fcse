...
def get_accuracy(y_true, naive_pred, tree_pred, forest_pred):
    total_correct = 0

    for i in range(len(y_true)):
        correct = 0
        if y_true[i] == naive_pred[i]:
            correct += 1

        if y_true[i] == tree_pred[i]:
            correct += 1

        if y_true[i] == forest_pred[i]:
            correct += 1
...
if __name__ == '__main__':
    ...
    class_two = [row for row in dataset if row[-1] == 2]

    naive_dataset = class_zero[:idx(class_zero, x1)] + class_one[:idx(class_one, x1)] + class_two[:idx(class_two, x1)]
    tree_dataset = class_zero[idx(class_zero, x1):idx(class_zero, x2)] + class_one[idx(class_one, x1):idx(class_one, x2)] + class_two[idx(class_two, x1):idx(class_two, x2)]
    forest_dataset = class_zero[:idx(class_zero, x2)] + class_one[:idx(class_one, x2)] + class_two[:idx(class_two, x2)]

    naive_train_x = [row[:-1] for row in naive_dataset]
    naive_train_y = [row[-1] for row in naive_dataset]
    naive_classifier = GaussianNB()
    naive_classifier.fit(naive_train_x, naive_train_y)

    tree_train_x = [row[:-1] for row in tree_dataset]
    tree_train_y = [row[-1] for row in tree_dataset]
    tree_classifier = DecisionTreeClassifier(random_state=0)
    tree_classifier.fit(tree_train_x, tree_train_y)

    forest_train_x = [row[:-1] for row in forest_dataset]
    forest_train_y = [row[-1] for row in forest_dataset]
    ...
    test_set = class_zero[idx(class_zero, x2):] + class_one[idx(class_one, x2):] + class_two[idx(class_two, x2):]

    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    naive_predictions = naive_classifier.predict(test_x)
    tree_predictions = tree_classifier.predict(test_x)
    forest_predictions = forest_classifier.predict(test_x)
