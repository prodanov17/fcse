...
if __name__ == '__main__':
    ...
    dataset_x, dataset_y = [[row[i] for i in range(len(row) - 1) if i != col_index] for row in dataset], [row[-1] for row in dataset]

    train_X, test_X, train_y, test_y = train_test_split(dataset_x, dataset_y, test_size=0.15, shuffle=False)

    model = RandomForestClassifier(n_estimators=num_trees, criterion=criterion, random_state=0)
