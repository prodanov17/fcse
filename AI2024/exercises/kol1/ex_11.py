...
class SnakeBoard:
    def __init__(self, size, green_apple_positions) -> None:
        self.size = size
...
class SnakeProblem(Problem):
    ...
    def result(self, state, action):
        ...
        return (snake.body, snake.direction, tuple(green_apples))
    ...
    def h(self, node):
        ...
        for (apple_x, apple_y) in green_apples:
            value += self.manhattan((apple_x, apple_y), (x, y))
            max_element = max(max_element, self.manhattan((apple_x, apple_y), (x, y)))
            min_element = min(min_element, self.manhattan((apple_x,apple_y),(x,y)))

        return min_element + len(green_apples) + (value // len(green_apples))
        # return value // len(green_apples)
...
if __name__ == "__main__":
    ...
    board = SnakeBoard(BOARD_SIZE, green_apple_positions)
    initial = (((0, 7), (0, 8), (0, 9)), 'down', tuple(green_apple_positions))
