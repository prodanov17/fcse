...
class SnakeProblem(Problem):
    ...
    def move(self, snake, direction):
        ...
        if direction == 'right':
            ...
            y -= 1
        new_snake = [(x, y)] + new_snake # return the snake whole, decide cutting later
        return tuple(new_snake)

    def change_direction(self, direction, action):
        if action == 'SvrtiLevo':
            if direction == 'up':
                direction = 'left'
            elif direction == 'down':
    ...
    def perform_action(self, state, action):
        snake, direction, apples = list(state)
        apples = list(apples)
        snake = list(snake)
        print(snake, direction, apples)
        if action == "ProdolzhiPravo":
            snake = self.move(snake, direction)
        elif action == "SvrtiLevo":
            direction = self.change_direction(direction, action)
            snake = self.move(snake, direction)
        elif action == "SvrtiDesno":
            direction = self.change_direction(direction, action)
            snake = self.move(snake, direction)
        if snake[0] in apples:
            ...
            snake = snake[:-1] # not eaten apple, remove the tail
        return (tuple(snake), direction, tuple(apples))
    ...
    def get_actions(self):
        return ["ProdolzhiPravo", "SvrtiDesno", "SvrtiLevo"]
...
if __name__ == "__main__":
    ...
    snake = ((0, 7), (0, 8), (0, 9))
    direction = ("down")
    initial_state = (snake, direction, tuple(zeleni_jabolki))
    snake = SnakeProblem(initial_state)
