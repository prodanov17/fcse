...
class Balls(Problem):
    def __init__(self, initial, goal, board_size, obstacles):
    ...
    def successor(self, state):
        ...
        for x, y in state:
            ...
            for action in actions:
                new_state = self.perform_action(state, action, (x, y))
    ...
    def perform_action(self, state, action, curr_pos):
        x, y = curr_pos
        new_state = list(state)
        new_state.remove((x, y))
        new_pos = (x, y)
        if action.startswith('Gore Levo'):
            if (x - 1, y + 1) in state:
                new_state.remove((x - 1, y + 1))
                new_pos = (x - 2, y + 2)
            else:
                return None
                new_pos = (x - 1, y + 1)

        elif action.startswith('Gore Desno'):
            if (x + 1, y + 1) in state:
                new_state.remove((x + 1, y + 1))
                new_pos = (x + 2, y + 2)
            else:
                return None
                new_pos = (x + 1, y + 1)
        elif action.startswith('Dolu Levo'):
            if (x - 1, y - 1) in state:  # check if there's a ball inbetween
                new_state.remove((x - 1, y - 1))
                new_pos = (x-2, y-2)
            else:
                return None
                new_pos = (x - 1, y - 1)
        elif action.startswith('Dolu Desno'):
            if (x + 1, y - 1) in state:
                new_state.remove((x + 1, y - 1))
                new_pos = (x + 2, y - 2)
            else:
                return None
                new_pos = (x + 1, y - 1)
        elif action.startswith('Levo'):
            if (x - 1, y) in state:
                new_state.remove((x - 1, y))
                new_pos = (x - 2, y)
            else:
                return None
                new_pos = (x - 1, y)
        elif action.startswith('Desno'):
            if (x + 1, y) in state:
                new_state.remove((x + 1, y))
                new_pos = (x + 2, y)
            else:
        ...
        if (new_pos) in state:
            return None
        new_state.insert(0, new_pos)
        # print()
        # print("START OF BLOCK")
        # print("STATE=", state)
        # print("ACTION=", action)
        # print("NEW_STATE=", new_state)
        # print("NEW_STATE=", tuple(new_state))
        # print("END OF BLOCK")
        # print()
    ...
    def get_actions(self, x, y):
        return [f"Gore Levo: (x={x},y={y})", f"Gore Desno: (x={x},y={y})", f"Dolu Levo: (x={x},y={y})",
                f"Dolu Desno: (x={x},y={y})", f"Levo: (x={x},y={y})", f"Desno: (x={x},y={y})"]
...
if __name__ == "__main__":
    ...
    initial_state = tuple(balls)
    goal_state = ((board_size//2, board_size-1))
    problem = Balls(initial_state, goal_state, board_size, tuple(obstacles))

    actions = ['Desno: (x=1,y=0)', 'Gore Desno: (x=3,y=0)', 'Gore Levo: (x=5,y=2)', 'Gore Desno: (x=3,y=4)', 'Levo: (x=5,y=6)']

    # state = initial_state
    # for action in actions:
    #     print("\n\n")
    #     print(problem.successor(state))
    #     print("\n\n")
